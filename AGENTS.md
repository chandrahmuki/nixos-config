...
| `aspects/*.nix` | Native Den aspects, auto-imported via `import-tree` |
| `aspects/compatibility.nix` | Explicit bridge for legacy modules not yet converted to native aspects |
| `modules/*.nix` | Legacy NixOS/HM modules loaded only through the compatibility aspect |
...
Each `aspects/<name>.nix` defines one or more Den aspects. New aspects are auto-imported by `import-tree` and must be included from the relevant host or user aspect.
...
1. `flake.nix` evaluates Den with `import-tree ./aspects`
2. `denHost.mainModule` contributes the selected host and user aspects to `nixosConfigurations.muggy-nixos`
3. `aspects/compatibility.nix` imports the remaining legacy modules explicitly
4. `overlays.nix` patches specific packages via `nixpkgs.overlays`
5. `nos` (nh) builds and activates the new generation
...
- **New configuration**: prefer a native `aspects/<name>.nix` aspect and include it from the relevant host or user aspect
...
| Question | Start here |
|----------|-----------|
| "How is configuration loaded?" | `aspects/den.nix`, `aspects/compatibility.nix`, then `flake.nix` |

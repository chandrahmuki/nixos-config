...
<system_commands>
  <command name="apply config">`just switch` ou `sudo nixos-rebuild switch --flake .` (sudo requis). Ne pas l'exécuter depuis l'éditeur.</command>
  <command name="flake update global">Pour tout mettre à jour SAUF le kernel CachyOS : `nix flake update nixpkgs home-manager niri noctalia antigravity`</command>
  <command name="flake update kernel">Pour mettre à jour uniquement le kernel : `nix flake update nix-cachyos`</command>
</system_commands>
